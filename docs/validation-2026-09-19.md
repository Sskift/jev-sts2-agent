...
| 指定 Opus 5 合成图请求 | 320×240 红矩形；ENOTFOUND，未收到 HTTP 响应；未换模型或关闭 TLS |
| 离线测试 | npm test：12/12 通过 |
...
剩余工作是修通本机 Claude Code 网关解析，验证真实视觉，补窗口裁剪/坐标变换与出牌后的变化确认，再进行一场战斗和房间推进。CLI mod 的本机兼容性、多阶段选牌、特殊资源、药水与高阶策略未验证。
