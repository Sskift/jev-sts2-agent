...
**当前追加阶段（2026-09-20）：补齐 Jev 的决策上下文。** 用户要求先完成本局状态、完整卡组/地图/战斗牌堆/历史及可选动作的 JSON 协议，再做实际测试。本项目在 `master` 直接开发提交，仅外部 CLI 模组考虑上游 PR。调研、实现、离线检查及模组编译已完成；新构建未部署，新一轮实机测试尚未开始。协议、记忆边界和测试记录见[决策上下文说明](docs/decision-context.md)。下文首场胜利是前一阶段的验收记录。
