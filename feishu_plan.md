...
当前部署模组为 `0.111.0-context.13`，针对本机游戏 v0.111.0 编译。119 项 Node 测试通过。已经完成过第一幕并到达第二幕 Boss；整局结果和具体问题见[实测进展](docs/full-run-progress.md)。测试通过不等于已经通关。

## 正在执行

1. 建设并验证决策管线：版本化本地规则库、当前状态优先、关联规则、最近动作与实际变化、完整合法候选旁的明确计算。用真实历史局面只读对照，随后进行新的实战验证；避免只靠逐张牌追加提示。
