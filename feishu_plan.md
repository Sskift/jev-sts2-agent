...
当前部署模组为 `0.111.0-context.14`，针对本机游戏 v0.111.0 编译；context.15 的 Cauldron 购买奖励通道修复已构建，等待主菜单部署。138 项 Node 测试通过。战斗增加回合目标、准备/收益依赖、有序行动、具体升级对象、条件变化后复核和有限完整序列比较。普通连续出牌沿用同一计划；不再默认每张牌独立决定。已经完成过第一幕并到达第二幕 Boss；整局结果和具体问题见[实测进展](docs/full-run-progress.md)。测试通过不等于已经通关。
