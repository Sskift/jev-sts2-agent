...
更新：2026-09-20。当前目标是 **Jev 在同一局标准游戏中从第一幕推进到第三幕，击败最终 Boss 并进入正式胜利结算**。只有完整通关、主分支提交和现有飞书文档同步全部完成，才结束 goal。允许正常失败后重新开始；不修改存档，不使用隐藏随机结果。

**当前执行计划与进展：**

- 完整决策 JSON 已在 `bc516b0` 实现；每次 Jev 请求独立提供当前状态、规则、地图、牌堆和相关历史，系统自己保存记忆。
- 已补齐奖励、商店、删牌、遗物、组合选牌和水晶球候选；循环继续普通奖励，正式结算才判断整局胜负。唯一合法动作直接执行。
- 83 项离线测试通过。已部署的上下文模组完成过第一幕并到达第二幕 Boss；尚未完成三幕通关。实测修复与各局结果见[整局进展](docs/full-run-progress.md)。
- 正在进行真实 Jev 整局测试，根据状态、动作和游戏表现修复阻塞；每次执行后重读，禁止重放结果不明的动作。
- 通关后更新运行说明、提交 `master`，同步[现有飞书文档](https://icnainlav1b8.feishu.cn/docx/Ijr1dLJpio6JvNxfcAfcnvXKnUR)，展示最终胜利画面。

本项目直接在 `master` 开发，不创建本项目 PR。协议和记忆背景见[决策上下文说明](docs/decision-context.md)。以下为前阶段的一场战斗验证记录与技术说明，涉及“一场后停止”的表述是历史原型限制，当前正在移除。
...
| 选择一步 | Jev | 自包含 `sts2.decision.v1` JSON（本局/玩家/卡组/地图/战斗/历史/合法动作）→ 一个候选 ID |
...
| `MAP` | 从 `travelable_coords` 枚举节点，结合玩家资源、完整构筑、后续图和路线统计选择；不再固定偏好普通战斗 |
