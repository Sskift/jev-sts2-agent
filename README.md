...
目前是研究原型。前 28 局中，16 局通过第一幕，最远到第二幕 Boss，尚未通关。第 28 局完成[复盘与上下文修正](docs/run28-review.md)后，第 29 局已击败第一幕 Ceremonial Beast，正在第二幕继续，使用 OpenRouter。已接入五个角色的策略参考、115 个怪物的行动图、独立有序候选和方案比较分歧记录；历史回放不能证明稳定通关或胜率提升。实时阶段记录见[整局进展](docs/full-run-progress.md)。
...
易伤的有序变化与 Slippery／Buffer 计数合并计算，确定的结果不会再被整段抹为未知。回收和洗牌节点也区分原始牌堆、此前计划出牌的通常去向，以及仍在结算的牌；例如先打防御再用 Headbutt 回收，可能是在准备下一回合，不能只看即时格挡。
