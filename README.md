...
目前是研究原型。30 局已结束，其中 18 局通过第一幕；最远记录仍为第二幕 Boss，尚未通关。第 30 局[复盘](docs/run30-review.md)后的修正已推送，第 31 局已从第一幕开始，正在进行。全程使用 OpenRouter；已接入五个角色的策略参考、115 个怪物的行动图、招式关联规则、独立有序候选和方案比较分歧记录。历史回放不能证明稳定通关或胜率提升。实时阶段记录见[整局进展](docs/full-run-progress.md)。
