...
目前是研究原型。已启动 35 局，34 局结束，22 局通过第一幕。最远记录仍为第二幕 Boss，尚未通关。第 35 局已击败 Ceremonial Beast，OpenRouter 服务恢复后继续推进到第二幕第 31 层；原生证据、费用和当前运行状态见[整局进展](docs/full-run-progress.md)。
...
战斗中有多个动作可选时，调用决策前按观察间隔再读取一次状态；事实或合法动作仍在变化就等待下一次观察，避免对尚未完成的结算付费规划。发送游戏命令前仍再次核对状态，两次稳定观察不保证后续不再变化。
