...
完整出牌方案的评分与比较使用各方案自己的有序预测，不再同时塞入所有孤立单动作的派生预测；全部原生预览、合法动作和规则仍保留。每批评分先核对所有请求容量再调用模型，维持 90 KB 本地上限和 24 项评分上限。第 45 局第二幕三只 Exoskeleton 的原生停点已通过[离线容量回放](docs/evidence/2026-09-24/run45-plan-budget.json)，未用模型费用或游戏动作来验证。
