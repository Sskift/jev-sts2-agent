...
目前是研究原型。已启动 37 局、36 局结束、22 局通过第一幕，尚未通关。第 37 局已加载 `31bf9d1`，正常从第一层开始。第 36 局在第一幕 Ceremonial Beast 第 11 回合失败，[复盘](docs/run36-review.md)记录营火取舍、商店预算和无攻击回合的过量格挡。最新修正将节费候选上限与方案呈现分开，同局面回放增加了有效输出，继续实战验证；原生证据和历次状态见[整局进展](docs/full-run-progress.md)。
