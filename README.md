...
目前是研究原型。累计开始 30 局，其中 18 局通过第一幕；第 30 局正在第二幕推进，最远记录仍为第二幕 Boss，尚未通关。第 29 局在双 Myte 战失败，完成[复盘与上下文修正](docs/run29-review.md)并推送后开始本局，全程使用 OpenRouter。已接入五个角色的策略参考、115 个怪物的行动图、独立有序候选和方案比较分歧记录；历史回放不能证明稳定通关或胜率提升。实时阶段记录见[整局进展](docs/full-run-progress.md)。
