...
目前是研究原型。已启动 38 局、37 局结束、22 局通过第一幕，尚未通关。第 38 局从第一层开始，沿用方案可读呈现、后续目标合法性和战斗完成边界修正。第 37 局在第一幕 Vantom 第 11 回合失败，[复盘](docs/run37-review.md)区分了准确的伤害核算、迟缓的破盾与构筑取舍；原生证据和历次状态见[整局进展](docs/full-run-progress.md)。
...
第 41 局第 11 层在 18/80 HP 时选择了升级，尽管 Rest 可回 24 HP；接下来一场战斗曾降至 5 HP。把该营地的原始状态及截至当时已确认的记忆放进离线回放后，新复核在交换选项顺序的两问中都选择 Rest；本局第 13 层 11 HP 时，新复核也把 Smith 改为 Rest。最后仍于第 17 层 Kin 首领战败，Priest 剩 42 HP。临终前 6 HP、0 能量时，原生预估结束回合会损失 12 HP 致死，唯一其他合法动作是喝 Entropic Brew 生成随机药水；旧规划仍结束回合。现在已知结束回合必死而有合法药水时，先使用一瓶并重新观察；同一历史局面的离线回放改为喝 Entropic Brew。随机生成的药水和后续胜负未被回放推定。
