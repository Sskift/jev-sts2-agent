...
目前是研究原型。30 局已结束，其中 18 局通过第一幕；最远记录仍为第二幕 Boss，尚未通关。第 30 局在第二幕第 29 层 The Obscura 战失败，正在依据[复盘](docs/run30-review.md)继续改进，尚未开始第 31 局。全程使用 OpenRouter；已接入五个角色的策略参考、115 个怪物的行动图、招式关联规则、独立有序候选和方案比较分歧记录。历史回放不能证明稳定通关或胜率提升。实时阶段记录见[整局进展](docs/full-run-progress.md)。
...
[策略知识](data/strategy/README.md) 将攻略建议与原生规则分开：每个组合记录前提和代价，不预设固定流派。怪物图区分固定顺序、条件、随机权重、重复限制和资料缺口；只根据可见意图关联后续可能行动，并关联已提取的生成牌规则、数量和目的地，保留阶段中断和识别歧义。
