...
联机接入：先在游戏内加入联机并进入一层，再运行 `npm start`。context.18 通过游戏的本机网络身份选择玩家，结束回合经原生网络动作队列同步，等待队友时持续观察。上下文包含队友公开状态与按玩家计算的意图预览；不读取队友手牌，不把全队攻击预览当成本机必受伤害。队友改变局面会使旧计划重新评估。已完成编译与离线检查，真实联机的出牌、同步和房间推进尚待验证；不会自动加入房间。

## 环境与启动

- Windows、Node.js（本机使用 v25.8.1）、已安装的游戏及兼容 CLI 模组。
- 当前支持游戏 **v0.111.0**，模组构建 **0.111.0-context.18**。安装与构建步骤见 [模组说明](mods/sts2-cli-compat/README.md)。只有重建模组或使用窗口诊断工具才需要 .NET 开发工具。
