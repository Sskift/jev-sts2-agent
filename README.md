...
目前是研究原型。此前 27 局中，16 局通过第一幕，最远到第二幕 Boss，尚未通关。完成本轮知识、续接资源与伤害账目修正后，已按更新后的目标开始第 28 局标准新游戏，使用 OpenRouter。已接入五个角色的策略参考、115 个怪物的行动图、独立有序候选和方案比较分歧记录；历史回放不能证明稳定通关或胜率提升。实时阶段记录见[整局进展](docs/full-run-progress.md)。
