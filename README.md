...
目前是研究原型。已启动并结束 35 局，22 局通过第一幕，尚未通关。第 35 局在第二幕第 33 层 The Insatiable 第 7 回合失败：80 生命、43 格挡仍被 Sandpit 倒计时处决。[复盘](docs/run35-review.md)区分删牌机会、抽牌稳定性与最后的死亡机制；原生证据和历次状态见[整局进展](docs/full-run-progress.md)。
