...
战后牌不再默认拿取。Jev 先评估每张牌相对于不加牌的增益；若同一奖励所有候选更可能属于恶化或边际收益，就按该评估跳过，否则对提议拿取的牌与跳过再作双向直接比较。第 38 局的[开发回放与攻略依据](docs/reward-choices.md)中，原本会拿的第二、第三张 Taunt 改为跳过，第一张仍拿；这是决策变化，尚不是通关率证据。
