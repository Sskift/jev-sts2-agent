...
目前是研究原型。已启动并结束 33 局，20 局通过第一幕。最远记录仍为第二幕 Boss，尚未通关。第 33 局全程使用 TypeSafe 官方直连，击败 The Kin 后在第二幕第 31 层失败；[复盘](docs/run33-review.md)补上抽牌等观察节点的已知受击缺口。实时记录见[整局进展](docs/full-run-progress.md)。
