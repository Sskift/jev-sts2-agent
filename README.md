...
目前是研究原型。已启动并结束 37 局、22 局通过第一幕，尚未通关。第 37 局在第一幕 Vantom 第 11 回合失败，[复盘](docs/run37-review.md)区分了准确的伤害核算、迟缓的破盾与构筑取舍。本局已加载方案可读呈现、后续目标合法性和战斗完成边界修正；原生证据和历次状态见[整局进展](docs/full-run-progress.md)。
