...
目前是研究原型。已启动 33 局，32 局已结束，20 局通过第一幕。最远记录仍为第二幕 Boss，尚未通关。第 32 局的[复盘](docs/run32-review.md)修正节段复活的条件表达后，第 33 局使用 TypeSafe 官方直连，在第 9 回合击败 The Kin 并进入第二幕。实时记录见[整局进展](docs/full-run-progress.md)。
