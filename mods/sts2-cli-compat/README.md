...
The build applies `decision-context.patch` after the original compatibility patch, and copies our `context/DecisionContextBuilder.cs` into the temporary source. The build then applies `multiplayer.patch`. It produces **0.111.0-context.18**, assembly/file version **0.111.0.19**. The original draft PR above only contains the v0.111 API compatibility changes; the decision-context extension is maintained in this repository.

context.18 resolves the local player through `LocalContext.GetMe`, including non-host clients. Combat readiness excludes dead or already-submitted players. Multiplayer End Turn queues `EndPlayerTurnAction` for network replication and returns on local readiness acknowledgement, without awaiting human teammates. Native target checks cover teammate players and pets. Party public state and recipient-dependent intent previews are exported; these previews do not establish actual enemy target assignment. Node invalidates plans on teammate changes, keeps waiting after local submission, and marks single-player outcome projections incomplete for multiplayer. Full native compilation and all 306 Node checks passed; live multiplayer remains unverified.
...
- `STS2.Cli.Mod.dll` and `STS2.Cli.Mod.json`, manifest version `0.111.0-context.18`, assembly version `0.111.0.19`.
