...
test('missing native Block uses only a complete unconditional rule, not part of a conditional effect', () => {
  ...
  assert.equal(result.hp_remaining_if_end_turn, null);
  assert.equal(result.block_preview.source, 'no_native_preview');
  const plain = combatForecast(combat, { ...card, description: 'Gain 25 Block.' });
  assert.equal(plain.block_after_card, 25);
  assert.equal(plain.hp_remaining_if_end_turn, 23);
  assert.equal(plain.block_preview.source, 'resolved_live_complete_rule');
...
test('a single-action estimate cannot claim a follow-up kill without an ordered plan', () => {
  const enemy = { combat_id: 1, hp: 16, block: 0, is_alive: true, powers: [{ id: 'SLIPPERY_POWER', amount: 1 }], intents: [{ damage: 8, hits: 1 }] };
  ...
  assert.equal(combatForecast(combat, strike, enemy).attack_hp_loss, 1);
  assert.equal(combatForecast(combat, strike, enemy).followup_attacks, undefined);
  assert.equal(combatForecast(combat, bigAttack, enemy).followup_attacks, undefined);
});

test('Orichalcum and delayed Rage block are not mistaken for immediate block gains', () => {
  const combat = { player: { hp: 10, energy: 3, block: 0, relics: [{ id: 'ORICHALCUM' }] }, hand: [], enemies: [{ is_alive: true, hp: 20, intents: [{ damage: 6, hits: 1 }] }] };
...
test('active Plating and Orichalcum use native turn-end timing, with no Dexterity or Frail scaling', () => {
  ...
  ], relics: [] }, hand: [], enemies: [{ is_alive: true, hp: 28, intents: [{ damage: 7, hits: 1 }] }] };
...
test('Rage exposes an affordable attack sequence as conditional Block without granting it immediately', () => {
  ...
  const combat = { player: { hp: 10, block: 0, energy: 3 }, enemies: [{ is_alive: true, hp: 100, intents: [{ damage: 13, hits: 1 }] }], hand: [rage, attack(1, 1), attack(2, 2), attack(3, 3), attack(4, 0), attack(5, 0, { hp_loss: 2 }), attack(6, -1), attack(7, 0, { can_play: false })] };
...
test('active Rage counts once per Attack, even at zero X, without Dexterity or Frail scaling', () => {
  ...
  ] }, hand: [], enemies: [{ is_alive: true, hp: 50, intents: [{ damage: 9, hits: 1 }] }] };
...
test('Second Wind counts other non-attacks and exposes exhausted setup cards', () => {
  ...
  const combat = { player: { hp: 9, block: 6, energy: 2 }, enemies: [{ is_alive: true, hp: 124, intents: [{ damage: 24, hits: 1 }] }], hand: [
...
test('an all-enemy attack uses each target preview and preserves surviving or unknown threats', () => {
  const enemies = [
    { combat_id: 1, hp: 6, block: 0, is_alive: true, intents: [{ damage: 10, hits: 1 }] },
    { combat_id: 2, hp: 6, block: 3, is_alive: true, intents: [{ damage: 7, hits: 1 }] },
    { combat_id: 3, hp: 6, block: 0, is_alive: true, intents: [{ damage: 4, hits: 1 }], powers: [{ id: 'SLIPPERY_POWER', amount: 1 }] },
    { combat_id: 4, hp: 6, block: 0, is_alive: true, intents: [{ damage: 2, hits: 1 }] }
  ];
  const combat = { player: { hp: 20, energy: 2, block: 0 }, hand: [], enemies };
  const area = { type: 'Attack', cost: 1, target_type: 'AllEnemies', target_previews: [1, 2, 3].map(target_id => ({ target_id, damage: 8 })) };
  const estimate = combatForecast(combat, area);
  assert.equal(estimate.hp_remaining_if_end_turn, null, 'The missing target preview is unknown, not zero damage');
  assert.deepEqual(estimate.attack_hp_loss_by_target.map(p => p.hp_loss), [8, 5, 1, null]);
  assert.deepEqual(estimate.attack_hp_loss_by_target.map(p => p.hp_depleted), [true, false, false, null]);
  assert.equal(combatForecast(combat, { ...area, target_type: 'RandomEnemy' }).hp_remaining_if_end_turn, null, 'An unknown random target cannot establish the remaining attack');
});

test('a playable zero-energy X attack must not falsely remove a lethal attacker', () => {
  const enemy = { combat_id: 1, hp: 5, block: 0, is_alive: true, intents: [{ damage: 20, hits: 1 }] };
  const whirlwind = { type: 'Attack', cost: -1, target_type: 'AllEnemies', can_play: true, target_previews: [{ target_id: 1, damage: 7 }] };
  ...
  assert.deepEqual(estimate.attack_hp_loss_by_target, [{ target_id: 1, hp_loss: null, hp_depleted: null }]);
  assert.equal(estimate.fatal_if_end_turn, null);
...
test('known repeated attacks include all hits but trigger active Rage only once', () => {
  const enemy = { combat_id: 1, hp: 7, block: 0, is_alive: true, intents: [{ damage: 20, hits: 1 }] };
...
test('explicit X hit previews distinguish zero hits, energy-paid hits and a visible X modifier', () => {
  const enemy = { combat_id: 1, hp: 9, block: 0, is_alive: true, intents: [{ damage: 10, hits: 1 }] };
...
test('an unmodeled Illusion revival cannot become a guaranteed follower knockdown', () => {
  const target = { combat_id: 2, hp: 21, block: 0, is_alive: true, powers: [{ id: 'ILLUSION_POWER', amount: 1 }], intents: [{ damage: 16, hits: 1 }] };
  ...
  assert.equal(forecast.followup_attacks, undefined);
...
test('declared self HP loss can kill before an otherwise fully blocked enemy turn', () => {
  const target = { combat_id: 2, hp: 61, block: 0, is_alive: true, intents: [{ damage: 6, hits: 1 }] };
...
test('Toxic hand damage uses block, and playing or exhausting the affected cards removes it', () => {
  ...
  const combat = { player: { hp: 19, energy: 3, block: 10 }, hand: [toxicA, toxicB], enemies: [{ is_alive: true, hp: 50, block: 0, intents: [{ damage: 18, hits: 1 }] }] };
