...
具名招式中的 `CreateCard<T>`、`ModelDb.Card<T>`、`ModelDb.Power<T>` 及已识别的能力调用另以 `rule_references` 关联规则，不推断施加结果。目前 92 种怪物的 139 个招式有 156 条引用，全部可在本地规则库解析。例如 The Insatiable 开场通过先创建再入堆的方式生成 Frantic Escape；现在地图上的可见 Boss 就能关联逃脱牌与 Sandpit 死亡规则。引用不代表本局已经获得卡牌或能力，也不确定初始数量、目的地或分支条件；其他辅助方法和未识别写法仍有缺口。
