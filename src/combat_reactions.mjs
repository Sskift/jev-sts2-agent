...
export function uncomputedAttackReactions(combat, card, target, hits) {
  ...
  return targets.flatMap(enemy => (enemy.powers || []).filter(power => power.id === 'THORNS_POWER' && power.amount > 0).map(power => ({
    source_id: power.id, owner_combat_id: enemy.combat_id, card_id: card.id,
    description: power.description, timing: 'before_each_qualifying_damage_instance',
    trigger: 'Powered attack damage or Omnislice damage, including a hit that would kill the owner; does not require HP damage to the owner.',
    ...
    scope: 'Native v0.111.0 Thorns hook; reaction damage, prevention, interruption on death and other hooks are not simulated. Preview hit count is conditional; unknown hits are not zero. Turn-end Block arrives after this reaction.'
  })));
}
