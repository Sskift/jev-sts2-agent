...
export function prepareStrategyAssessment(prepared, options) {
  ...
  const questions = Object.fromEntries([...assessmentChoices].map(([id, { action_id, candidate }]) => [id, {
    type: 'score',
    instructions: `Evaluate this specific option's incremental contribution to winning the entire run: ${candidate.description} Its action_id is ${action_id}. Compare taking it now against not taking it, using the whole current deck, existing copies, actual costs, supported triggers, relics, potions, HP and visible route. Account for both offense and sustained defense, boss scaling, draw and energy. Check analysis.deck_statistics.early_damage_check when present: a first efficient Attack can replace slow starter damage in the next fights, while an extra redundant Attack can delay stronger cards. This is a concrete output comparison, not a quota. Judge the effect's real frequency and payoff, not keyword overlap. Strong defense or damage can be useful even when draw could also improve. Do not assume additional future cards will provide missing support. ${candidate.request.cmd.startsWith('shop_') ? shopComparison : ''} This rating is advisory, not an action. ${references}`,
    criteria: levels
  }]));
  const payload = { model: prepared.payload.model, state: prepared.payload.state, questions };
  const body = JSON.stringify(payload), bytes = compileModelRequest(payload, { purpose: 'option_assessment' }).bytes, max = options.maxRequestBytes ?? JEV_REQUEST_BUDGET;
